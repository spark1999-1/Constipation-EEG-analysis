"""
Perform inter-group statistical analysis on EEG features (topological features), Fig 7
"""
...
def feature_boxplot(task_data, task_name, feature, p_all):
    """
    Draw a boxplot comparison chart of specific features across different groups and frequency bands
    """

    groups = task_data['Group'].unique().tolist()
    bands = task_data['Band'].unique()
    ...
    for pair_key in pairs:
        ...
        p_val = p_all.loc[condition, 'P_Raw']
        p_val = p_val.iloc[0]
...
def auc_calculate_glo(thresholds, results):
    n_subjects= len(results[thresholds[0]])
    ...
    for subject in range(n_subjects):
        ...
        for key, value in results.items():
            all_value.append(value[subject])

        # calculate AUC
        area = trapezoid(all_value, thresholds)
        normalized_auc = area / threshold_range
        auc_results.append(normalized_auc)
...
def run_ancova(data, feature_names):
    """
    Run ANCOVA
    """
    results_list = []
    ...
    return df_results.sort_values(by='Cohen_d')
...
if __name__ == "__main__":
    ...
    healthy_ages = [57, 54, 58, 50, 51, 50, 54, 52, 55, 55, 52, 46, 49, 47, 54, 51, 54, 54, 31, 22, 30, 29, 29, 26, 26,
                    30, 22, 40, 25, 34, 21, 26, 37, 22, 35, 38, 25]
    ...
    for type_i in type:
        ...
        for threshold in thresholds:
            dir_path = '../data/network_feature_' + str(threshold) + '/' + type_i + '/'
                ...
                for task_name, value_task in value_all.items():
                    for band_name, band_value in value_task.items():
                        for feature_name, feature_value in band_value.items():
                            all_features0[type_i][task_name][band_name][feature_name][threshold].append(feature_value)

        # Calculate the AUC values at different thresholds
        for task_name, value_task in all_features0[type_i].items():
            for band_name, band_value in value_task.items():
                for feature_name, feature_value in band_value.items():
                    network_auc = auc_calculate_glo(thresholds, feature_value)
                    all_features[type_i][task_name][band_name][feature_name] = network_auc
    ...
    # Plot topological features' differences between groups
    tasks = df['Task'].unique()
    df_results = run_ancova(data=df.copy(), feature_names=feature_names)

    for task in tasks:
        task_data = df[df['Task'] == task]
        for feature in feature_names:
            fig = feature_boxplot(task_data, task, feature, p_all=df_results)